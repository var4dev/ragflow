...
def _remap_agent_dsl_for_tenant(dsl: dict, source_tenant_id: str, target_tenant_id: str) -> tuple[dict, list[dict]]:
    ...
    def _get_default_fallback_id(tenant_id: str, src_type, src_name: str | None) -> str | None:
        """Return default model id for tenant as fallback when exact match missing."""
        # Map binary type to LLMType value
        fallback_type = None
        if src_type is not None:
            try:
                ...
                for lt in LLMType:
                    try:
                        if calculate_model_type(lt.value) & int(src_type):
                            fallback_type = lt.value
                            break
                    except Exception:
                        continue
            except Exception:
        ...
        if not fallback_type:
            # Heuristic: embedding/rerank names contain those keywords, else chat
            low = (src_name or "").lower()
            if "embed" in low:
                fallback_type = LLMType.EMBEDDING.value
            elif "rerank" in low:
                fallback_type = LLMType.RERANK.value
            elif "asr" in low or "speech" in low:
                fallback_type = LLMType.ASR.value
            elif "vision" in low or "img2txt" in low:
                fallback_type = LLMType.VISION.value
            elif "tts" in low:
                fallback_type = LLMType.TTS.value
            else:
                fallback_type = LLMType.CHAT.value
        try:
            exist, tenant = TenantService.get_by_id(tenant_id)
            if exist and tenant:
                field_map = {
                    LLMType.CHAT.value: "tenant_llm_id",
                    LLMType.EMBEDDING.value: "tenant_embd_id",
                    LLMType.RERANK.value: "tenant_rerank_id",
                    LLMType.ASR.value: "tenant_asr_id",
                    LLMType.VISION.value: "tenant_img2txt_id",
                    LLMType.TTS.value: "tenant_tts_id",
                }
                field = field_map.get(fallback_type)
                if field:
                    did = getattr(tenant, field, None)
                    if did:
                        ok, m = TenantModelService.get_by_id(did)
                        if ok and m:
                            return did
                # Last resort: any active model of that type or chat
                for lt_val in [fallback_type, LLMType.CHAT.value]:
                    try:
                        for prov in TenantModelProviderService.get_by_tenant_id(tenant_id) or []:
                            for inst in TenantModelInstanceService.get_all_by_provider_id(prov.id) or []:
                                for m in TenantModelService.get_models_by_instance_id(inst.id) or []:
                                    try:
                                        ...
                                        if calculate_model_type(lt_val) & int(getattr(m, "model_type", 0)):
                                            return m.id
                                    except Exception:
                                        continue
                    except Exception:
                        ...
                        warnings.append(
                            {
                                "type": "model",
                                "source_id": llm_id,
                                "logical": logical or llm_id,
                                "reason": f"not found on target, fallback to default {fb}",
                                "fallback": True,
                            }
                        )
                    else:
